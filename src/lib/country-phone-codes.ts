...
/** ITU calling codes — sorted A→Z by country name at export */
const countryPhoneCodesRaw: CountryPhoneCode[] = [
  ...
  { iso: "VI", name: "Virgin Islands (US)", dialCode: "+1" },
  { iso: "YE", name: "Yemen", dialCode: "+967" },
  { iso: "ZM", name: "Zambia", dialCode: "+260" },
  { iso: "ZW", name: "Zimbabwe", dialCode: "+263" },
];

export const countryPhoneCodes: CountryPhoneCode[] = [...countryPhoneCodesRaw].sort((a, b) =>
  a.name.localeCompare(b.name, "en"),
);

export const defaultCountryIso = "IN";

export function getCountryByIso(iso: string): CountryPhoneCode | undefined {
  return countryPhoneCodes.find((c) => c.iso === iso);
}

export function countrySelectLabel(country: CountryPhoneCode): string {
  return `${country.name} (${country.dialCode})`;
}

/** Combine selected country dial code with local number for validation/submit */
export function composeInternationalPhone(countryIso: string, localNumber: string): string {
...
export function phonePlaceholderForCountry(countryIso: string): string {
  const country = getCountryByIso(countryIso);
  if (!country) return "+XX XXXXX XXXXX";
  if (country.iso === "IN") return "9876543210";
  if (country.dialCode === "+1") return "555 123 4567";
  return "Phone number";
}
