    ...
    >
      ...
      <div className="flex h-14 w-full items-center justify-between px-4 lg:hidden">
        <span data-nav="logo" className="mx-nav-pending inline-flex shrink-0">
          <BrandLogo size="navMobile" href="/" priority />
        </span>
        <button
          type="button"
          className="flex h-11 w-11 min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-lg text-[var(--v6-text)] hover:bg-white/10"
      ...
      <div className="hidden w-full lg:block">
        <div className="v6-container v6-nav-inner">
          <span data-nav="logo" className="mx-nav-pending inline-flex shrink-0">
            <BrandLogo size="header" href="/" priority />
          </span>
          ...
          <div className="relative" ref={resourcesRef}>
            ...
            {resourcesOpen ? (
              <ul
                id="resources-menu"
                className="max-h-[min(70vh,22rem)] overflow-y-auto overscroll-contain rounded-2xl border border-[var(--v6-border)] bg-[#12121c] py-2 shadow-xl"
                ...
                {resourceLinks.map((link) => (
                  <li key={link.href} role="none">
                    <Link
                      href={link.href}
                      role="menuitem"
                      className="block px-4 py-2.5 text-sm font-medium text-[var(--v6-text)] transition-colors hover:bg-white/5 hover:text-violet-300"
          ...
          <div className="flex shrink-0 items-center gap-2">
            <a
              ...
              className="mx-nav-pending inline-flex items-center gap-1.5 rounded-full border border-[#25D366] px-3 py-1.5 text-xs font-semibold text-[#128C7E] transition hover:bg-[#25D366]/10"
              data-nav="cta"
            ...
            <button
              type="button"
              className="mx-nav-pending inline-flex items-center gap-1 rounded-full bg-violet-600 px-3.5 py-1.5 text-xs font-bold text-white transition hover:bg-violet-500"
              data-nav="cta"
              onClick={() => {
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 right-0 z-50 flex w-[min(100%,20rem)] flex-col border-l border-[var(--v6-border)] bg-[#0e0e16] lg:hidden",
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--v6-border)] px-5 py-4">
          ...
          <button
            type="button"
            onClick={closeMobile}
            className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg text-sm font-medium text-slate-400 hover:bg-white/10"
        ...
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-3 py-4" aria-label="Mobile navigation">
            ...
            <Link
              ...
              className={cn(
                ...
                isActive(link.href)
                  ...
                  : "text-[var(--v6-text-secondary)] hover:bg-white/5",
              )}
            >
              {link.label}
            </Link>
          ...
          {mobileServiceShortcuts.map((shortcut) => (
            <Link
              ...
              href={shortcut.href}
              onClick={closeMobile}
              className="rounded-xl px-4 py-3 text-sm font-semibold text-violet-300 hover:bg-violet-600/5"
            >
              {shortcut.label}
            </Link>
          ))}
          ...
          {servicesNavGroups.map((group) => (
            <div key={group.title}>
              <button
                type="button"
                className="flex min-h-[44px] w-full items-center justify-between px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-400"
                onClick={() =>
                ...
                <div className="pb-2 pl-4">
                  {group.links.map((link) => (
                    <Link
                      ...
                      href={link.href}
                      onClick={closeMobile}
                      className="flex min-h-[44px] items-center gap-2 px-2 py-3 text-sm text-slate-300 hover:text-violet-300"
                    >
                      {link.label}
                    </Link>
                  ))}
          ...
          {resourceLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={closeMobile}
              className="rounded-xl px-4 py-3 text-[var(--v6-text-secondary)] hover:bg-white/5"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="space-y-3 border-t border-gray-100 p-4">
          <button
            type="button"
            className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-xl bg-violet-600 py-3.5 text-base font-semibold text-white hover:bg-violet-500"
            onClick={() => {
          ...
          <Link
            ...
            className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl border border-white/15 py-3 text-sm font-semibold text-slate-300"
            onClick={() => {
          ...
          <a
            href={telHref}
            className="flex min-h-[44px] w-full items-center justify-center gap-2 py-2 text-sm text-slate-400"
          >
            📞 {siteConfig.phone}
          </a>
        </div>
