...
export function AuthorCard({ author, variant = "inline" }: { author: Author; variant?: "inline" | "full" }) {
  ...
  return (
    <Card href={`/authors/${author.slug}`} padding="lg">
      <div className="flex items-start gap-4">
        ...
        <div>
          <H3 className="group-hover:text-brand-400 transition-colors">{author.name}</H3>
          <Caption className="text-brand-500">{author.role}</Caption>
          <Caption className="mt-1">{author.position}</Caption>
          <Caption className="mt-3 leading-relaxed line-clamp-3">{author.bio}</Caption>
          ...
          <Caption className="mt-4 text-brand-600 group-hover:underline">View profile →</Caption>
        </div>
      </div>
    </Card>
  );
